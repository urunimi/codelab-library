...
package com.codelab.library.widget;
...
import com.codelab.library.graphics.RotateBitmap;
...
//		}
		setImageRotateBitmapResetBase(new RotateBitmap(bitmap, rotation), false);
	}
	...
	/** 
	 * This function changes bitmap, reset base matrix according to the size of the bitmap, 
	 * and optionally reset the supplementary matrix.
	 * @param bitmap
	 * @param resetSupp
	 */
	...
	public void setImageRotateBitmapResetBase(final RotateBitmap bitmap, final boolean resetSupp) {
		...
		if (bitmap.getBitmap() != null) {
			...
			super.setImageBitmap(bitmap.getBitmap());
			
			Bitmap old = mBitmapDisplayed.getBitmap();
			mBitmapDisplayed.setBitmap(bitmap.getBitmap());
			mBitmapDisplayed.setRotation(bitmap.getRotation());
			if (old != null && old != bitmap.getBitmap() && mRecycler != null) {
				mRecycler.recycle(old);
			}
		} else {
			mBaseMatrix.reset();
