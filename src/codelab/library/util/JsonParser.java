...
public class JsonParser {
			...
			try {
				if(result.has(fieldName)) {
					if(int.class.equals(fieldType)) {
						...
						f.set(object, getNewList(gType.getClass(), result.get(fieldName).toString()));
				...
				LogByCodeLab.w(f.getType() + " instance failed. " + result.get(f.getName()).toString());
			}
		}
		LogByCodeLab.v(type.getSimpleName() + " instance created. " + object.toString());
	...
	public static<T extends Object> List<T> getNewList(Class<T> type, String jsonString) throws Exception {
		if(jsonString != null) {
			...
			LogByCodeLab.v(type.getSimpleName() + " list created. size is " + list.size());
